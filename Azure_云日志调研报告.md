...
Azure 上也能搜索到 Audit log，这个实际上就是 Entra ID 的 Audit log，内容是一样的，为了查询方便整合在 Azure Portal 里面了。
...
P.S. Azure Activity Log 的设计初衷是**“审计（谁动了我的资源）”而非“监控（谁看了我的资源）”**。

写操作 (Write/Action)
所有的 PUT, POST, DELETE 操作都会被强制记录。

例子： 创建虚拟机、修改防火墙规则、重置密码、给资源打标签。

重要的读操作 (Specific Read/Action)
这是 Azure 与 AWS 最大的区别点。Azure 默认不记录普通的“查看”操作（如 List 或 Get），但会记录涉及安全敏感信息的读操作。

例子： 列出存储账户的访问密钥 (List Keys)。虽然这是一个“读”动作，但它拿到了权限，所以 Activity Log 会记录一条 Microsoft.Storage/storageAccounts/listKeys/action。

对比 AWS： CloudTrail 默认记录大量的 DescribeInstances、ListBuckets 等操作。在 Azure 里，如果你只是在 Portal 上刷了一下虚拟机列表，Activity Log 是保持安静的。
