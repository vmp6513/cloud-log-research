...
**关键设计约束**：
- 优先设计**消息队列监听（Push-Pull）**的方案，客户在自己的账号将日志推送到某个队列（或类似的服务）中，通知部署在 SaaS 平台上订阅的后台程序进行日志采集。次优先考虑通过 **SDK/HTTP 拉取（Pull）** 主动定时拉取的方案进行采集。
- 采集方案在正常配置下需满足**准实时（5-15分钟）**端到端延迟（事件发生 → 后台可消费）目标。
...
| **数据事件（Data Events）** | S3 GetObject/PutObject、Lambda 调用等数据面操作；按量收费 | ✅ | ❌ **不支持** |
| **Insights 事件** | 异常 API 调用速率检测结果 | ✅ | ❌ 不支持 |

> **选型关键约束**：若客户需要采集数据事件（如 S3 对象访问审计、Lambda 调用溯源），**必须使用方案一**（Trail → S3），方案二无法覆盖此场景。
...
| **方案二**：Trail → EventBridge → SQS（轻量方案） | CloudTrail → EventBridge → SQS → 后台拉取 | **3–10 分钟** | ⭐（低，约 3 步） | EventBridge 极低费用；**无需 S3** | 客户不想自建 S3 日志桶，或只需采集特定高危管理事件 |
...
此方案跳过 S3 存档环节，通过 EventBridge 将 CloudTrail 事件**近实时推送**到 SQS，后台直接消费完整事件 JSON。适合客户不想自建 S3 日志桶、或只需对特定高危管理事件做精准采集的场景。
...
| 适用事件类型 | 管理事件 + 数据事件 | **仅管理事件**（CloudTrail 与 EventBridge 的集成仅覆盖管理事件，见 §2.2） |
...
> 关键字段含义与方案一（§4.4）相同，但事件内容在 `detail` 层级下，解析时需先拆信封。`time` 字段是 EventBridge 收到事件的时间，研判时应使用 `detail.eventTime` 作为事件时间基准。
