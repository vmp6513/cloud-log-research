...
> **与 AWS 的关键差异：源侧过滤缺失及其工程影响**
>
> Azure 诊断设置本身**不支持字段级过滤**，只能按日志 Category（如 `Administrative`、`Security`）整体开关，无法像 AWS EventBridge 规则那样在数据源头按 `operationName`、`caller`、`resultType` 等字段精确筛选，只推送关心的事件子集。
>
> 这一限制的直接工程影响是：**Azure 场景下不存在"只采高危事件"的轻量路径。**
>
> 在 AWS 方案二（EventBridge）中，客户可以在账号侧配置规则，使 SQS 队列里只有 IAM 高危操作、CloudTrail 自身被篡改等少量核心事件，后台消费的消息量可以压缩到全量的 1%–10%，后台吞吐和存储压力极低。Azure 做不到这一点——无论选哪套方案，后台都必须**全量接收 Activity Log（乃至 Resource Logs）后，在消费侧自行过滤**。
>
> 这对后台架构有以下连锁影响，产品侧和研发团队需要在设计阶段就考虑到：
>
> | 影响维度 | 具体表现 |
> |---|---|
> | **后台吞吐压力** | 中型企业（~1000 人）Activity Log 约 5 万–20 万条/天；若客户开启 Key Vault、Storage 等高频 Resource Logs，日均事件量可达百万级。后台必须具备对应的全量消费能力，而非按需消费 |
> | **存储与计算成本** | 全量数据入库后再过滤，存储量和计算量均远高于 AWS EventBridge 精确过滤路径；若产品侧希望"按事件类型分级存储"，过滤逻辑必须完全在 SaaS 后台实现 |
> | **噪音比** | 高价值安全事件（如 `roleAssignments/write`）在 Activity Log 全量数据中占比极低，告警规则必须在后台做精确的 `operationName` 匹配，否则误报率极高 |
> | **功能设计约束** | 未来若产品侧希望推出"客户自定义采集范围"功能（如"只采 IAM 相关事件"），Azure 无法将过滤下推到客户侧配置；该功能在 Azure 上只能在 SaaS 后台侧实现，与 AWS 的实现路径有本质差异 |
>
> **结论**：Azure 的无源侧过滤特性不是可以靠方案选型绕过的限制，而是平台能力边界。后台架构必须按"全量数据流入、消费侧过滤"的模型设计，容量规划应以全量日志规模为基准，不能参照 AWS EventBridge 方案的轻量假设。
...
> **⚠️ 产品与架构设计提示：Azure 无源侧过滤能力对后台设计的影响**
>
> 无论选择上述哪套方案，Azure 的诊断设置均不支持字段级过滤（详见 §2.1），这意味着：
>
> - **后台永远是全量数据流入**：Event Hub 或 Storage Account 中的数据是 Activity Log 全量（及开启的 Resource Logs 全量），不存在"只推送高危事件"的配置选项
> - **所有过滤逻辑必须在 SaaS 后台消费侧实现**：告警规则、事件分级、采集范围控制等功能，在 Azure 上只能是后台能力，无法像 AWS 那样部分下推到客户侧配置
> - **容量规划基准**：后台的吞吐、存储、Worker 并发设计，必须以全量日志规模为基准（中型客户百万级/天量级），不能参照 AWS EventBridge 精确过滤后的轻量假设
>
> 这是 Azure 与 AWS 采集架构最需要在产品设计阶段对齐的差异点。详细的影响分析见 §2.1。
