...
本章从**客户（运维/开发人员）视角**出发，说明"客户需要在自己的 AWS 账号里做什么，才能让我们的采集后台拿到日志"。
...
基于准实时（5–15 分钟）的产品定位，我们提供三套采集方案，客户根据自身环境和接受的配置复杂度选择：
...
| **方案三**：Trail → S3（仅存档，后台定时拉取） | CloudTrail → S3 → 后台定时轮询 | **10–30 分钟** | ⭐（低，约 3 步） | 仅 S3 存储费 | 对实时性要求低；最简单配置 |

> 三套方案均兼容授权模型 A（跨账号 AssumeRole）和模型 B（AKSK），授权配置见 §3.1。

**业界使用情况参考**：

三套方案的链路均为业界成熟做法，非本文定制方案，以下为主流安全厂商的公开文档佐证：
...
> **重要限制**：EventBridge 与 CloudTrail 的原生集成目前**仅覆盖管理事件**，数据事件（S3 GetObject 等）无法通过此链路采集。若客户需要数据事件，必须使用方案一或方案三。
...
> 相比方案一，方案二**无 S3 存储成本**，但**失去了历史日志留存能力**。若客户有合规要求需保留 90 天以上日志，必须选择方案一或方案三，或同时保留一条写入 S3 的 Trail。

---

### 3.5 方案三：Trail → S3（定时轮询，最简配置）

此方案是方案一的简化版：去掉 SNS/SQS 通知链路，后台改为定时主动轮询 S3 新文件。配置最简单，但延迟略高。

**与方案一的核心区别**：

| 对比项 | 方案一（SNS+SQS 通知） | 方案三（定时轮询） |
|---|---|---|
| 延迟 | 5–15 分钟（SNS 通知驱动） | 10–30 分钟（轮询间隔决定） |
| 客户配置步骤 | 5 步 | **3 步**（Trail + S3 + 授权） |
| 新文件感知方式 | 事件驱动（SNS 推送，无延迟） | 时间驱动（后台每 N 分钟扫一次 S3） |
| 漏拉风险 | 低（SQS 有消息确认机制） | 需后台自维护"已拉取进度"状态 |

**适用客户**：不想配置 SNS/SQS，且对 10–30 分钟延迟可接受的客户，或仅用于事后调查的场景。

#### 3.5.1 客户配置步骤

**第 1 步**：参照方案一第 1 步，创建 Multi-Region Trail 并写入 S3（相同操作）。

**第 2 步**：参照方案一第 4 步，为 SaaS 后台授予 S3 读取权限（去掉 SQS 相关权限）。

**第 3 步**：在 SaaS 平台填写 S3 Bucket 名称、账号 ID、授权信息。后台自动以 5 分钟为间隔扫描 S3 目录，对比上次已拉取的文件名列表，增量下载新文件。

#### 3.5.2 成本说明

成本与方案一相同，但**不含 SNS/SQS 费用**（本来就可忽略不计）。

---

### 3.6 方案对比与选型建议

| 维度 | 方案一（Trail→S3→SQS） | 方案二（EventBridge→SQS） | 方案三（Trail→S3 轮询） |
|---|---|---|---|
| **延迟** | 5–15 分钟 | 3–10 分钟 | 10–30 分钟 |
| **客户配置步骤** | 5 步，中等复杂 | 3 步，简单 | 3 步，最简单 |
| **事件覆盖** | 管理 + 数据事件 | 仅管理事件 | 管理 + 数据事件 |
| **历史日志留存** | ✅ 有（S3）| ❌ 无 | ✅ 有（S3） |
| **数据源侧过滤** | ❌ 不支持 | ✅ 支持（EventBridge 规则） | ❌ 不支持 |
| **SQS 积压导致丢失** | **可补救**（消息超期只丢通知，S3 文件仍在，后台可补偿扫描） | **不可补救**（消息超期 = 事件永久丢失，无任何补救途径） | 不适用（无 SQS） |
| **DLQ 必要性** | 建议配置 | **强烈建议，是唯一补救手段** | 不需要 |
| **客户侧月度增量成本** | S3 存储费（低） | EventBridge 费（极低） | S3 存储费（低） |
| **推荐客户类型** | 需要全量日志的主流客户 | 只需高危事件、配置越简单越好 | 实时性要求低、配置最简单 |
...
客户是否需要数据事件（S3 GetObject 等）？
    ...
    └─ 否 → 继续
          客户是否对配置复杂度敏感？
              ├─ 是（越简单越好）→ 方案二（注意：需强制配置 DLQ，无 S3 兜底）
              └─ 否（接受多步配置）→ 方案一（推荐：有 S3 兜底，丢通知可补偿）
```
