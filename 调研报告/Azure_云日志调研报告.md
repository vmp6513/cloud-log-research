...
### 3.1 高权限 RBAC 角色分配（Administrative 类别）
...
### 3.3 Key Vault 密钥异常访问（Resource Log）
...
### 3.4 跨订阅横向移动（RBAC 角色分配，不同 subscriptionId）
...
### 5.1 案例一：APT29 通过 Azure 服务主体和 RBAC 滥用实施持久化横向移动（CISA，2021）
...
### 5.2 案例二：供应链攻击路径——通过 Managed Identity 实现订阅间横向移动
